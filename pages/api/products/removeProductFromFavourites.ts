...
export type RemoveProductFromFavouritesRequest = { productId: string };

export const path = "api/products/removeProductFromFavourites";

export const addProductToFavourites: NextApiHandler<RemoveProductFromFavouritesRequest> = async (req, res) => {
  ...
  const requestBody = req.body.params as RemoveProductFromFavouritesRequest;
  ...
  await prisma.user.update({ where: { email: session.user.email }, data: { favourites: { disconnect: { id: product.id } } } });
